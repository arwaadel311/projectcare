...
export const signupDoctor = asyncHandler(async (req, res, next) => {
    ...
    if (req.files?.unionCard?.length) {
    
        for (const file of req.files.unionCard) {

            const { secure_url, public_id } = await cloudinary.uploader.upload(req.files?.unionCard[0].path, { folder: `${process.env.APP_NAME}/doctor/unionCard` })
            req.body.unionCard={ secure_url, public_id }
...
export const getDataHeartbeat = asyncHandler(async (req, res, next) => {
    ...
    const doctor = await doctorModel.findOne({patientId:patientId})
    if (!doctor) {
        return next(new Error('In-valid account', { cause: 400 }))
    }
   // console.log(doctor);
//    if (OnePatientDocID.heartRate > 130 || OnePatientDocID.heartRate <50 && OnePatientDocID.motionRate>2) {
...
//    <tr>
//    <td>
//    <h1 style="padding-top:25px; color:#630E2B">${OnePatientDocID.firstName} has a seizure</h1>
...
//    <tr>
//    <td>
//    <h3 style="margin-top:10px; color:#000">notification doc</h3>
//    </td>
//    </tr>
//    <tr>
//    <td>
...
//    </body>
//    </html>`
// if (!await sendEmail({ to: doctor.email, subject: 'notificationDoc', html })) {
...
return res.status(200).json({ message: "Done" ,OnePatientDocID,doctor})
